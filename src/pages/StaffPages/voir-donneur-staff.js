...
const BasicTable = props => {
  ...
  const handleEditDonateur = async () => {
    try {
      ...
      setModalOpen9(false) // Fermer le modal après la modification réussie
    } catch (error) {
  ...
  return (
    <React.Fragment>
      ...
      <Modal isOpen={modalOpen9} toggle={() => setModalOpen9(!modalOpen9)}>
        <ModalHeader toggle={() => setModalOpen9(!modalOpen9)}>
          Modifier Donateur
        </ModalHeader>
        <ModalBody>
        ...
        <ModalFooter>
          <Button color="secondary" onClick={() => setModalOpen9(!modalOpen9)}>
            Fermer
          </Button>
